...
## スコープ宣言
- このディレクトリで扱う: ムームーレインボーのホテル配送エリアマップ（Leaflet地図・WP埋め込み用HTML・ホテル一覧マスタ）のみ
- 扱わない: PKハワイ Shopify EC（`~/muumuu-pkhawaii/` へ）、muumuurainbow.jp 本体等のWP保守（該当作業場所で）、他クライアント案件、Panthers横断秘書業務（`~/ai-management/` へ）

## 秘書連携ルール（必須）
重要決定・進捗・期日変更・キーパーソン変更（エリア定義・料金変更等）が発生したら、ローカルメモリだけでなく **ハブ側 `~/ai-management/00_context/memories/clients/ムームーレインボー.md` にも追記**すること（AI鈴木さんが全体把握に使う）。地図描画の微調整など実装の試行錯誤はローカルのみでOK。
